# codex-rewind-hard-truncation Specification

## Purpose

定义 Codex rewind 的真实截断语义：成功返回必须代表锚点后事实已经从运行时会话与持久化 session 文件中同时被移除，且 reopen / history load / send 等后续生命周期链路只能读取截断后真值。
