...
`ConversationAssembler` MUST serve as the shared semantic assembly boundary for `Codex`, `Claude Code`, and `Gemini` realtime observations and history snapshots before conversation-visible state is consumed by lifecycle-aware clients.
...
- **WHEN** a `Gemini` history loader returns parsed assistant, reasoning, tool, or user items
- **THEN** the system MUST assemble that snapshot through `ConversationAssembler.hydrateHistory()` or an equivalent shared assembler entrypoint
...
Normalized realtime input MUST have a reusable assembly boundary so adapter outputs can be converged without reinterpreting raw engine payload semantics. This boundary MUST be available for `Codex`, `Claude Code`, and `Gemini` main realtime adapter outputs.
